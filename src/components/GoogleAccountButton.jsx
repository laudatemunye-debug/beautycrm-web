...
const S = {
  ...
  disconnectBtn: { marginLeft:'auto', padding:'4px 8px', background:'transparent', border:'1px solid #e0e0e0', borderRadius:6, fontSize:11, color:'#888', cursor:'pointer', flexShrink:0 },
...
export const GoogleAccountButton = () => {
  const { googleUser, syncing, authReady, error, connect, disconnect } = useGoogle();
    ...
    return (
      <div>
        <div style={S.card}>
          ...
          <button style={S.disconnectBtn} onClick={disconnect} title="Deconnecter ce compte">
            Deconnecter
          </button>
        </div>
  ...
  return (
    <div>
      <button style={S.connectBtn} onClick={connect} disabled={!authReady || syncing}>
        <GoogleLogo />
